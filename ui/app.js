...
function khClock(){const n=new Date(),k=new Date(n.getTime()+7*36e5),p=v=>String(v).padStart(2,"0");
 let diff="";try{const mins=tz=>{const o={};new Intl.DateTimeFormat("en-CA",{timeZone:tz,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hourCycle:"h23"}).formatToParts(n).forEach(x=>o[x.type]=+x.value);return Date.UTC(o.year,o.month-1,o.day,o.hour,o.minute)/6e4};
  const h=Math.round((mins("Asia/Phnom_Penh")-mins("Europe/Paris"))/60);if(h>0)diff=` <span class="khd">· ${h} h de plus qu'à Paris</span>`}catch(e){}
 return `<i aria-hidden="true"></i>Il est <b>${p(k.getUTCHours())}:${p(k.getUTCMinutes())}:${p(k.getUTCSeconds())}</b> au Cambodge <span class="khu">· UTC+7</span>${diff}`}
const khBar=()=>`<div class="khbar" id="khbar" title="Heure locale du Cambodge (Phnom Penh)">${khClock()}</div>`;
setInterval(()=>{const el=document.getElementById("khbar");if(el){const h=khClock();if(el.innerHTML!==h)el.innerHTML=h}},1000);
...
function nav(active){return `<nav class="top" aria-label="Navigation principale"><div class="wrap"><a class="logo" href="#home" aria-label="Le Guide Cambodge, accueil"><i></i>Le Guide<span style="font-weight:300;color:var(--mute)">&nbsp;Cambodge</span></a>
...
function home(){const m=new Date().getMonth();const seasons=["dry","dry","hot","hot","hot","rain","rain","rain","rain","rain","dry","dry"];
 ...
 return khBar()+nav("home")+`<header class="hero" id="top">${ph(PH.home,"hero")}<div class="wrap"><span class="eyebrow" style="color:var(--sand)">Royaume du Cambodge · 25 provinces</span>
